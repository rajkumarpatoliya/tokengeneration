...
contract('DappToken', function(accounts) {
    ...
    it('transfers token ownership', function(){
        return DappToken.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._from,accounts[0], 'logs account, the tokens are transferred from');
            assert.equal(receipt.logs[0].args._to,accounts[1], 'logs account, the tokens are transferred to');
    ...
    it('approves tokens for delegated transfer', function() {
        return DappToken.deployed().then(function(instance){
            ...
            assert.equal(allownace.toNumber(), 100, 'it stores the allowance for delegated transfer');
        });
    });
    ...
    it('Handles delegated token transfers', function() {
        return DappToken.deployed().then(function(instance) {
            ...
            // Try transferring something larger then the sender's balance
            return tokenInstance.transferFrom(fromAccount, toAccount, 9999, {from: spendingAccount});
        }).then(assert.fail).catch(function(error){
            assert(error.message.indexOf('revert') >= 0, 'cannot transfer value larger than balance');
            // Try transferring something larger then approved amount
            ...
            assert.equal(receipt.logs[0].event, 'Transfer', 'should be the "Transfer" event');
            assert.equal(receipt.logs[0].args._from, fromAccount, 'logs account, the tokens are transferred from');
